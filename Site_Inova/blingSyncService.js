...
let isProductLucasSyncRunning = false;
let isProductElianeSyncRunning = false;
...
async function syncBlingProductsLucas() {
    ...
    isProductLucasSyncRunning = true;
    ...
    try {
        ...
        isProductLucasSyncRunning = false;
...
async function syncBlingProductsEliane() {
    ...
    isProductElianeSyncRunning = true;
    ...
    try {
        ...
        isProductElianeSyncRunning = false;
        shouldStopNFeLucas = false;
        shouldStopNFeEliane = false;
        client.release();
...
async function syncNFeLucas() {
    ...
    try {
        ...
        const pagina = 1; // MODIFICADO: Busca apenas a primeira página.

        // Não há mais loop `for (let pagina = 1; pagina <= 5; pagina++)`
        ...
        try {
            ...
            if (nfeListResponse.data.length === 0) {
                 console.log(`[LucasSync] Nenhuma NF-e encontrada na página ${pagina}.`);
            } else {
                 for (const nfeResumo of nfeListResponse.data) {
                    // Verificações de interrupção
                    ...
                    try { // Bloco TRY para cada NFe individual
                        ...
                        // MODIFICADO: Apenas verifica se a NFE já existe no cache. Se existir, pula para a próxima.
            ...
            console.log(`[LucasSync] Página ${pagina} de NFs sincronizada.`);
        ...
        // Lógica original para verificação de notas canceladas
        await new Promise(resolve => setTimeout(resolve, 500));
        try {
...
async function syncNFeEliane() {
    ...
    try {
        ...
        try {
            ...
            if (nfeListResponse.data.length === 0) {
                console.log(`[ElianeSync] Nenhuma NF-e encontrada na página ${pagina}.`);
            } else {
                 for (const nfeResumo of nfeListResponse.data) {
                    ...
                    try { // Bloco TRY para cada NFe individual
                        ...
                        // MODIFICADO: Apenas verifica se a NFE já existe no cache.
                        const checkCacheResult = await pool.query(
                            'SELECT 1 FROM cached_nfe WHERE chave_acesso = $1',
                            [chaveDeAcesso]
                        );
                        ...
                        if (nfeDetalhes.numeroPedidoLoja) {
                            try {
                                ...
                                if (pedidoSearchResponse.data && pedidoSearchResponse.data.length > 0) {
                                    ...
                                    console.log(`   [ElianeSync] Pedido ${p.numero} (NF: ${nfeDetalhes.numero}) salvo no cache.`);
                                }
                            } catch (pedidoError) {
            ...
            console.log(`[ElianeSync] Página ${pagina} de NFs sincronizada.`);
