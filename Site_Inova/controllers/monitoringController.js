...
exports.bulkAddProductsFromFile = async (req, res) => {
    ...
    try {
        ...
        const insertQuery = `
            INSERT INTO urls (url, description, sku)
            VALUES ($1, $2, $3);
        `;
  ...
  try {
    const urlsResult = await pool.query(`
      SELECT u.sku, u.description, u.custo,
             ...
             (SELECT MIN(price) FROM dados_produtos WHERE url_id = u.id AND seller != 'inova móveis') AS concorrentePrice
      FROM urls u
      LEFT JOIN dados_produtos dp ON u.id = dp.url_id AND dp.seller = 'inova móveis'
    `);
...
exports.generateMonitoringReport = async (req, res) => {
    ...
    try {
        ...
        const productsToMonitor = urlsData.map(urlItem => {
            ...
            const inovaMoveisData = productPrices.find(pd => pd.seller === 'inova móveis');
            const inovaMoveisPrice = inovaMoveisData ? parseFloat(inovaMoveisData.price) : null;
            const lowestPriceData = productPrices.filter(pd => pd.seller !== 'inova móveis').sort((a, b) => parseFloat(a.price) - parseFloat(b.price))[0];
...
exports.getMonitoringProducts = async (req, res) => {
  try {
    ...
    const productsToMonitor = urlsData.map(urlItem => {
      ...
      const inovaMóveisData = productPrices.find(pd => pd.seller === 'inova móveis');
...
exports.getProductsOutOfPromotion = async (req, res) => {
  try {
    ...
    res.render('monitoring/productsOutOfPromotion', {
...
exports.getNonCompetitiveProducts = async (req, res) => {
  try {
    ...
    `);
    res.render('monitoring/non-competitive-products', {
...
exports.editUrl = async (req, res) => {
  ...
  try {
    ...
    res.render('monitoring/edit-url', { 
