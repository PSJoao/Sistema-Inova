                ...
                try {
                    ...
                    if (nfeDetails.itens && nfeDetails.itens.length > 0) {
                        ...
                        const cachedProductsResult = await pool.query(
                            ...
                            [skusDaNota, report.bling_account_type]
                        );
                        
                        // Mapeia SKU -> Objeto completo (para acessar volumes e id)
                        const cachedProductsMap = new Map(cachedProductsResult.rows.map(p => [p.sku, p]));
                        ...
                        for (const item of nfeDetails.itens) {
                            ...
                            if (cachedProductsMap.has(produtoCodigo)) {
                                ...
                                volumesUnit = cachedProd.volumes || 0;
                                productId = cachedProd.bling_id; // Pega do cache
                            } else {
                                try {
                                    ...
                                    if (prodSearchResp.data?.[0]?.id) {
                                        productId = prodSearchResp.data[0].id; // Pega da API
                                        
                                        const prodDetailsResp = await blingApiGet(`https://api.bling.com.br/Api/v3/produtos/${productId}`, report.bling_account_type);
                    ...
                    const productIdsListStr = idsBlingProduto.join('; '); // [NOVO] String final de IDs
                    // --- FIM DA LÓGICA DE VOLUMES E IDS ---
                    ...
                    await pool.query(
                        `INSERT INTO cached_nfe (
                                bling_id, bling_account, nfe_numero, chave_acesso, transportador_nome, total_volumes, 
                                product_descriptions_list, product_ids_list, 
                                data_emissao, 
                         ...
                         ON CONFLICT (nfe_numero) DO NOTHING`, 
                        [
                            nfeDetails.id, 
                            ...
                            productIdsListStr, // [NOVO] $8 passa a ser a lista de IDs
...
exports.getPrintLabelsPage = async (req, res) => {
    ...
    console.log(`BLING ID: ${nfeBlingIds}`);
    ...
    try {
        ...
        // Busca NFs
        const nfeQuery = `
            SELECT cn.bling_id, cn.nfe_numero, cn.chave_acesso, cn.transportador_nome, cn.bling_account, cn.fone,
                   cn.etiqueta_nome, cn.data_emissao, cn.etiqueta_endereco, cn.etiqueta_numero, cn.etiqueta_complemento, 
                   cn.etiqueta_municipio, cn.etiqueta_uf, cn.etiqueta_cep, cn.etiqueta_bairro, cn.product_ids_list, pv.numero
            FROM cached_nfe cn LEFT JOIN cached_pedido_venda pv ON cn.nfe_numero = pv.nfe_parent_numero
            WHERE cn.bling_id = ANY($1::bigint[])
        `;
        const nfeResult = await client.query(nfeQuery, [nfeBlingIds]);

        console.log(`LINHAS DE CONSULTA: ${nfeResult.rows}`);
        ...
        console.log(`NOTA FISCAL ENCONTRADA: ${allNfeNumeros}`);
        const quantQuery = `
