...
async function atualizarStatusPedidosSsw(pedidosSsw) {
    ...
    for (const pedido of pedidosSsw) {
        try {
            ...
            // Mesmo se status_manual for true, ainda atualizamos rastreio e datas
            const deveAtualizarSituacao = !pedido.status_manual;
            const marcarParaConferencia =
                !pedido.status_manual && pedido.situacao_atual !== situacaoNovaPadronizada;

            // Extrai previsão de entrega
            ...
            // Extrai data de entrega se código = 01
            let dataEntrega =
                String(situacaoMaisRecente.codigo_ssw) === '01'
                    ? situacaoMaisRecente.data_hora_efetiva
                    : null;
            ...
            if (dataEntrega) {
                fieldsToUpdate.push(`data_entrega = $${queryParams.push(dataEntrega)}`);
            }

            if (dataPrevisaoEntrega && !pedido.previsao_atu) {
                fieldsToUpdate.push(
                    `data_previsao_entrega = $${queryParams.push(dataPrevisaoEntrega)}`
                );
            }
            ...
            console.error(
                `[SSW Service] Erro ao consultar API para chave ${pedido.chave_nfe}:`,
                error.message
            );
        }
    }
