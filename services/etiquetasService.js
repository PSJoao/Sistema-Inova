...
async function getInfoPorNFe(nfeNumero) {
    ...
    try {
        ...
        if (!nfeData && String(nfeNumero).length < 6) {
            let nfeNumeroPadded = String(nfeNumero).padStart(6, '0');
            nfeData = await fetchNfeFromCache(client, nfeNumeroPadded);
            if (nfeData) nfeNumero = nfeNumeroPadded;
        }
        ...
        if (nfeData) {
            ...
            let nfeFoiEncontrada = await findAndCacheNfeByNumber(nfeNumero, 'lucas', false);
            let accountUsed = 'lucas';

            if (!nfeFoiEncontrada) {
                console.log(`   [Bling Fail] NFe ${nfeNumero} não encontrada na conta lucas. Tentando eliane...`);
                let nfeNumeroEliane = String(nfeNumero).padStart(6, '0');
                nfeFoiEncontrada = await findAndCacheNfeByNumber(nfeNumeroEliane, 'eliane', false);
                accountUsed = 'eliane';
                if (nfeFoiEncontrada) {
                    nfeNumero = nfeNumeroEliane; // Atualiza a variável para buscar no cache corretamente
                }
            }

            if (nfeFoiEncontrada) {
                console.log(`   [Bling Success] NFe ${nfeNumero} encontrada na conta ${accountUsed} e cache atualizado pela fila.`);
                nfeData = await fetchNfeFromCache(client, nfeNumero);
            } else {
                console.warn(`   [Bling Fail] NFe ${nfeNumero} não foi encontrada no Bling (lucas e eliane).`);
                return null;
            }
        }
...
async function fetchNfeFromCache(client, nfeNumero) {
    const nfeResult = await client.query('SELECT chave_acesso, product_ids_list, etiqueta_cep FROM cached_nfe WHERE nfe_numero = $1 ORDER BY bling_account = \'lucas\' DESC LIMIT 1', [nfeNumero]);
...
async function processarLoteNf(excelBuffer) {
    ...
    const notFoundNfs = [];
    const loadedPdfs = new Map(); // caminho do arquivo -> PDFDocument
    ...
    try {
        // Busca todas as NFe de uma só vez para evitar consultas individuais no banco
        const queryResult = await client.query(
            `SELECT nfe_numero, pdf_arquivo_origem, pdf_pagina
             FROM cached_etiquetas_ml
             WHERE nfe_numero = ANY($1)`,
            [nfList]
        );
        dbRows = queryResult.rows;
    } catch (dbErr) {
        console.error('[Service - Lote NF] Erro ao buscar informações no banco:', dbErr);
    } finally {
        client.release();
    }

    // Mapeia os dados do banco por nfe_numero para busca rápida O(1)
    const dbMap = new Map();
    dbRows.forEach(row => {
        dbMap.set(row.nfe_numero, row);
    });

    for (const nf of nfList) {
        try {
            const dbData = dbMap.get(nf);

            if (!dbData) {
                console.log(`[Service - Lote NF] NF ${nf} não encontrada no banco.`);
                notFoundNfs.push(nf);
                continue;
            }

            let arquivoOrigem = dbData.pdf_arquivo_origem;
            const pdfPagina = dbData.pdf_pagina;

            if (!arquivoOrigem || arquivoOrigem === 'desconhecido' || pdfPagina === null || pdfPagina === undefined) {
                console.warn(`[Service - Lote NF] NF ${nf}: Informações de arquivo ou página inválidas/ausentes no DB.`);
                notFoundNfs.push(nf);
                continue;
            }
            ...
            let pdfDoc;
            if (loadedPdfs.has(filePath)) {
                pdfDoc = loadedPdfs.get(filePath);
            } else {
                // Verifica se o arquivo existe no disco
                try {
                    await fs.access(filePath);
                } catch (accessError) {
                    console.error(`[Service - Lote NF] NF ${nf}: O arquivo ${arquivoOrigem} não foi encontrado em ${PDF_STORAGE_DIR}.`);
                    notFoundNfs.push(nf);
                    continue;
                }

                console.log(`[Service - Lote NF] Lendo arquivo ${arquivoOrigem} do disco...`);
                const pdfBytes = await fs.readFile(filePath);
                pdfDoc = await PDFDocument.load(pdfBytes);
                loadedPdfs.set(filePath, pdfDoc);
            }

            if (pdfPagina >= pdfDoc.getPageCount()) {
                console.error(`[Service - Lote NF] NF ${nf}: Índice da página (${pdfPagina}) inválido para o arquivo ${arquivoOrigem} que tem ${pdfDoc.getPageCount()} páginas.`);
                notFoundNfs.push(nf);
                continue;
            }

            const [copiedPage] = await mergedPdf.copyPages(pdfDoc, [pdfPagina]);
            mergedPdf.addPage(copiedPage);
            foundCount++;

        } catch (error) {
            console.error(`[Service - Lote NF] Erro ao buscar/processar NF ${nf}:`, error);
            notFoundNfs.push(nf);
        }
    }
    ...
    console.log(`[Service - Lote NF] Mesclando ${foundCount} etiquetas encontradas...`);
...
async function syncBlingConferenciaMassa(nfeList) {
    ...
    try {
        ...
        for (const nfeNumero of nfeList) {
            ...
            // Pausa rigorosa de 50ms entre as chamadas para respeitar o rate-limit do Bling
        ...
        for (let i = 0; i < etiquetasOrdenadas.length; i++) {
            ...
            try {
                if (etiqueta._tipo === 'pdf') {
                    // === ETIQUETA MANUAL (PDF) — busca direta via cache em memória ===
                    let arquivoOrigem = etiqueta._pdfArquivoOrigem;
                    const pdfPagina = etiqueta._pdfPagina;

                    if (!arquivoOrigem || arquivoOrigem === 'desconhecido' || pdfPagina === null || pdfPagina === undefined) {
                        console.warn(`[Impressão] NF ${etiqueta.nfeNumero}: Informações de arquivo ou página inválidas/ausentes no DB.`);
                        continue;
                    }
                    ...
                    if (loadedPdfs.has(filePath)) {
                        ...
                        try {
                            ...
                            console.error(`[Impressão] NF ${etiqueta.nfeNumero}: O arquivo ${arquivoOrigem} não foi encontrado em ${PDF_STORAGE_DIR}.`);
                            continue;
                        }

                        console.log(`[Impressão] Lendo arquivo ${arquivoOrigem} do disco...`);
                        const pdfBytes = await fs.readFile(filePath);
                        pdfDoc = await PDFDocument.load(pdfBytes);
                        loadedPdfs.set(filePath, pdfDoc);
                    }
                    ...
                    if (pdfPagina >= pdfDoc.getPageCount()) {
                        console.error(`[Impressão] NF ${etiqueta.nfeNumero}: Índice da página (${pdfPagina}) inválido para o arquivo ${arquivoOrigem} que tem ${pdfDoc.getPageCount()} páginas.`);
                        continue;
                    }
    ...
    if (zplContent) {
        ...
        for (let attempt = 1; attempt <= 5; attempt++) {
            try {
                ...
                if (attempt < 5) {
                    await new Promise(res => setTimeout(res, 150)); // Pausa padronizada de 150ms
                }
            }
        }
