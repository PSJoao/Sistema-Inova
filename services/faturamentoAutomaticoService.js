...
async function atualizarNotaNoBling(nfeOriginal, novosValores, token) {
    ...
    const payload = {
        ...
        transporte: {
            ...
            // >>> CORREÇÃO: Força o envio dos dados de volume originais (quantidade, peso) para não resetar
            volume: nfeOriginal.transporte.volume,
            volumes: nfeOriginal.transporte.volumes
        },
        despesas: 0, 
        desconto: 0, 
    ...
    for (const notaResumo of notas) {
        ...
        try {
            ...
            if (nfeDetalhes.itens && nfeDetalhes.itens.length > 0) {
                 // Itera sobre cada item original e aplica os 70% individualmente
                 for (const item of nfeDetalhes.itens) {
                     // Cálculo direto: Valor Unitário Original * 0.70
                     const unitarioCalculado = item.valor * 0.70;
                     
                     novosValores.itens.push({
                         novoValorUnitario: parseFloat(unitarioCalculado.toFixed(4)) // Mantém 4 casas decimais para precisão
                     });
                 }
            } else {
            ...
            if (putSuccess) {
                try {
                    console.log(`[Faturamento Auto] NFe ${nfeDetalhes.numero} faturada e atualizada com sucesso no Bling. Enviando ao Sefaz...`);
                    const url = `${BLING_API_BASE_URL}/nfe/${nfeDetalhes.numero}/enviar`;
                    await axios.post(url, {}, {
                        headers: { 'Authorization': `Bearer ${token}` }
                    });

                    console.log(`[Faturamento Auto] NFe ${nfeDetalhes.numero} enviada com sucesso!`);
                } catch (error) {
                    console.log(`[Faturamento Auto] Erro ao enviar a nota ${nfeDetalhes.numero} para o sefaz`, error.message);
                } finally {
                    await processingClient.query('COMMIT');
                }
            } else {
